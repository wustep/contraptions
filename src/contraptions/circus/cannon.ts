...
export const cannon = defineContraption({
  ...
  draw: (p, s, { size: k, w, h, u, ink, weight }) => {
    ...
    // The acrobat: shot, caught, tipped onto the ramp, rolled home.
    let pos: [number, number] | null = null
    if (u >= SHOT && u < 0.78) pos = flight(EXIT, CATCH, 0.4, seg(u, SHOT, 0.78))
    ...
    clipBox(p, w, h, () => {
      ...
      // The fuse, what is left of it, and the spark.
      if (u >= LIT && u < SHOT) {
        const n = FUSE.length - 1
        const at = (1 - burn) * n
        const i = Math.min(n - 1, Math.floor(at))
        const f = at - i
        const spark: [number, number] = [lerp(FUSE[i][0], FUSE[i + 1][0], f), lerp(FUSE[i][1], FUSE[i + 1][1], f)]
        outline(p, ink, weight)
        p.beginShape()
        p.vertex(FUSE[0][0] * k, FUSE[0][1] * k)
        for (let j = 1; j <= i; j++) p.vertex(FUSE[j][0] * k, FUSE[j][1] * k)
        p.vertex(spark[0] * k, spark[1] * k)
        p.endShape()
        knob(p, k, ink, weight, s.color, spark[0], spark[1], 0.07 + 0.02 * Math.sin(u * 200))
      }
