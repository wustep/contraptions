 ...
 */
const Y_TOP = -0.7
/** The ring floor stands clear of the tent's ground line, so the bucket's
 *  tray is still inside the footprint when the bucket is down. */
const Y_BOT = 0.9
...
export const bigTop = defineContraption({
  ...
  draw: (p, s, { size: k, u, ink, weight }) => {
    ...
    ground(p, k, 2, 1)
    ground(p, k, 2, Y_BOT)
    for (const x of [-0.86, 0.86]) stroke(p, k, x, Y_BOT, x, 1)
    ...
    stroke(p, k, FALL_X - TUBE, -0.97, FALL_X - TUBE, ARC_C[1])
    ...
    stroke(p, k, GATE_X - 0.08, -0.94, GATE_X + 0.08, -0.94)
    p.push()
    p.translate(GATE_X * k, -0.94 * k)
    p.rotate(gate)
    block(p, k, ink, weight, s.color, 0, 0.125, 0.07, 0.25)
    p.pop()
    knob(p, k, ink, weight, s.color, GATE_X, -0.94, 0.06)
  },
})
