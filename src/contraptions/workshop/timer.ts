...
export const timer = defineContraption({
  ...
  draw: (p, s, { size: k, u, ink, weight }) => {
    ...
    // The plunger in its sleeve, sprung back against the dial's rim. At rest
    // the head sits against the sleeve; at full shove it slides through and
    // its tip lands on the east wall, not in the bench next door.
    outline(p, ink, weight)
    p.rect(0.4 * k, CY * k, 0.08 * k, 0.14 * k)
    p.line(0.4 * k, (CY + 0.07) * k, 0.4 * k, BENCH * k)
    p.line(tipX * k, CY * k, (0.38 + ext) * k, CY * k)
