 ...
 * loop, reaches the keg, and the bang is what kicks the run off, after which
 * the spool pays out a fresh fuse for next time.
 */
const FIRE = 0.66
...
export const fuse = defineContraption<Beat>({
  ...
  draw: (p, s, { size: k, u, ink, weight }) => {
    ...
    if (t < BANG) {
      const f = t / BANG
      p.push()
      p.translate(LID[0] * k, (LID[1] - 0.06) * k)
      p.stroke(s.color)
      p.strokeWeight(weight)
