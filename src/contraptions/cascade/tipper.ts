 ...
 * overbalances, and the dump washes the waiting ball off the rail and away
 * down the run. The tray swings back empty and starts filling again.
 *
 * The ball waits under the lip for exactly `emit` and leaves on the pour, so
 * the head of the line always has one ready to go.
 */
const FIRE = 0.7
const PIVOT_X = -0.2
const PIVOT_Y = -0.04
const TRAY_H = 0.24
/** The tray is tipped by this much, which lands its lip on the ball. */
const TIP = 1.7
/** The pour and its splash. */
const POUR = 0.12
/** When the tray is back upright and the drips start counting again. */
const REFILL = 0.36
/** Where the ball waits, clear of the tray and under its lip. */
const SEAT = 0.13
...
export const tipper = defineContraption<Beat>({
  ...
  // The pour lands on the ball and the splash comes off it, so both are drawn
  // after the world's tokens rather than behind them.
  over: (p, s, { size: k, u, weight }) => {
    const t = since(u, FIRE)
    clipCell(p, k, () => {
      if (t > 0.02 && t < POUR) {
        ...
        p.line(-0.02 * k, -0.22 * k, SEAT * k, (-0.14 + 0.06 * f) * k)
        p.pop()
      }
      if (t > 0.04 && t < POUR + 0.06) {
        ...
        p.noStroke()
        p.fill(s.color)
        const throwX = 0.06 + 0.28 * f
        const throwY = 0.3 * 4 * f * (1 - f)
        const d = lerp(0.09, 0.03, f)
        p.circle((SEAT - throwX) * k, (-0.04 - throwY) * k, d * k)
        p.circle((SEAT + throwX) * k, (-0.04 - throwY) * k, d * k)
        p.pop()
      }
    })
