...
async function main(): Promise<void> {
  ...
  check('in the picker the works are Clair de Lune, Cornfield Chase, Epilogue, Everything, Gymnopédie, Married Life, Première Arabesque and Voyage',
    shipped.works.map((w) => w.title).sort().join('|') === 'Clair de Lune|Cornfield Chase|Epilogue|Everything|Gymnopédie|Married Life|Première Arabesque|Voyage', shipped.works.map((w) => w.title).join('|'))
  check('no take carries a byline', shipped.works.every((w) => w.versions.every((v) => !('director' in v))))
  check('the shows are Clair de Lune, Come Recover, Cornfield Chase, Gymnopédie, Interstellar, La La Land, Married Life and Première', shipped.works.map((w) => w.work).sort().join(',') === 'clair-de-lune,come-recover,cornfield-chase,gymnopedie,interstellar,la-la-land,married-life,premiere-arabesque')
