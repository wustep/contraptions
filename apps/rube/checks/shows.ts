...
import epilogueOnsets from '../../../scripts/shows/plans/lalaland-epilogue-onsets.json'
...
async function main(): Promise<void> {
  ...
  check('in the picker the works are Clair de Lune, Cornfield Chase, Epilogue, Everything, Gymnopédie, Mountain King, Première Arabesque and Voyage',
    shipped.works.map((w) => w.title).sort().join('|') === 'Clair de Lune|Cornfield Chase|Epilogue|Everything|Gymnopédie|Mountain King|Première Arabesque|Voyage', shipped.works.map((w) => w.title).join('|'))
  check('no take carries a byline', shipped.works.every((w) => w.versions.every((v) => !('director' in v))))
  check('the shows are Clair de Lune, Come Recover, Cornfield Chase, Gymnopédie, Interstellar, La La Land, Mountain King and Première', shipped.works.map((w) => w.work).sort().join(',') === 'clair-de-lune,come-recover,cornfield-chase,gymnopedie,interstellar,la-la-land,mountain-king,premiere-arabesque')
  const mountainKing = shipped.works.find((w) => w.work === 'mountain-king')?.versions ?? []
  check('mountain-king is Mountain King, its take Spark, with no note',
    mountainKing.map((v) => v.take).join(',') === 'opus55-spark' && mountainKing.every((v) => v.title === 'Mountain King' && v.note === undefined) && mountainKing.map((v) => v.label).join('|') === 'Spark')
