...
/**
 * The kit every part of Married Life is built from (after Liftoff's and Epilogue's: the same contract, so a slot is struck by construction).
 *
...
export interface Built<S> {
  ...
  changes?: BallChange[]
  /**
   * Ellie, wherever this part shows her (`worlds.ts`, ELLIE; the stage never draws her, `cast.ts` does). Each span
   * is in show seconds and may run past the part's own slot (she can still be sitting in the yard while he is inside
   * fetching the book); at any time at most one part in the show has her. Positions are in the part's frame; null
   * while she is out of sight, and only when she is out of shot or at a cut.
   */
  company?: Company[]
  /**
   * How Carl holds himself, where the default will not do. `cast.ts` tilts him with the slope he is moving along and
   * keeps him upright in the air; a span here says otherwise (a lean, a bow, a slump) in show seconds.
   */
  pose?: Pose[]
}

/** Where she is: the ball's own fields but its id and, unless the part says otherwise, its colour (her age's). */
export type Companion = Omit<ShowBall, 'id' | 'color'> & { color?: string }

/** Who keeps Carl company. Ellie, the only one. */
export type Who = 'ellie'

/** A stretch of show time in which a part has her. */
...
/**
 * Carl's bearing over a stretch of show time: `tilt` (radians, clockwise on the screen) replaces the one he would
 * take from the slope; `squash` (0 none, 0.3 a lot) flattens him onto what he stands on, as a landing does, keeping
 * his bottom where it is.
 */
export interface Pose {
  from: number
  to: number
  at: (t: number) => { tilt?: number; squash?: number }
}
...
type Styled = { drawingContext: CanvasRenderingContext2D; _cachedFillStyle?: unknown; _cachedStrokeStyle?: unknown; _setFill?: (f: unknown) => void; _setStroke?: (s: unknown) => void; lifeHonest?: boolean }
/**
 ...
 * and paint with the gradient instead (a dark that came out beige). So the renderer Married Life draws with always sets
 * the style it is given.
 */
function honest(p: p5): void {
  const r = (p as unknown as { _renderer?: Styled })._renderer
  if (!r || r.lifeHonest || !r._setFill || !r._setStroke) return
  r.lifeHonest = true
...
export interface Chain {
  ...
  company: Company[]
  /** The parts' spans of Carl's bearing (no moving needed: a tilt is a tilt anywhere). */
  pose: Pose[]
...
export function lay(start: { col: number; row: number; begin: number; ball: BallState }, links: Link[]): Chain {
  ...
  for (const link of links) {
    ...
    if (Math.abs(time - span) > 1e-6) console.warn(`married life: ${link.part.piece.name} lasts ${time.toFixed(4)}s of a ${span.toFixed(4)}s slot`)
    const end = laneAt(built.lane, time)
    if (Math.hypot(end.x - (built.exit[0] - 0.5), end.y - built.exit[1]) > 1e-6) console.warn(`married life: ${link.part.piece.name} ends at ${end.x.toFixed(3)},${end.y.toFixed(3)}, not at its exit`)
  ...
  return { placed, shots, company, pose, next: { col, row, begin, ball } }
}
