...
/**
 * The kit every part of Epilogue is built from (after Liftoff's kit; the same contract, so a slot is struck by construction).
 *
...
export interface Built<S> {
  ...
  riders?: Riders
  /**
   * The hero's company: Mia (`worlds.ts`, MIA) or, at the end, her
   * husband (HUSBAND), wherever this part shows them. Each span is in show seconds
   * and may run past the part's own slot (a table in the club still has
   * her at it while the hero is inside the piano); at any time at most
...
/** Who keeps the hero company: Mia (the default), or her husband. */
export type Who = 'mia' | 'husband'
...
type Styled = { drawingContext: CanvasRenderingContext2D; _cachedFillStyle?: unknown; _cachedStrokeStyle?: unknown; _setFill?: (f: unknown) => void; _setStroke?: (s: unknown) => void; epilogueHonest?: boolean }
/**
 ...
 * and paint with the gradient instead (a dark that came out beige). So the renderer Epilogue draws with always sets
 * the style it is given.
 */
function honest(p: p5): void {
  const r = (p as unknown as { _renderer?: Styled })._renderer
  if (!r || r.epilogueHonest || !r._setFill || !r._setStroke) return
  r.epilogueHonest = true
...
export function lay(start: { col: number; row: number; begin: number; ball: BallState }, links: Link[]): Chain {
  ...
  for (const link of links) {
    ...
    if (Math.abs(time - span) > 1e-6) console.warn(`epilogue: ${link.part.piece.name} lasts ${time.toFixed(4)}s of a ${span.toFixed(4)}s slot`)
    const end = laneAt(built.lane, time)
    if (Math.hypot(end.x - (built.exit[0] - 0.5), end.y - built.exit[1]) > 1e-6) console.warn(`epilogue: ${link.part.piece.name} ends at ${end.x.toFixed(3)},${end.y.toFixed(3)}, not at its exit`)
