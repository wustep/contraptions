...
*Epilogue* is a suite. It is the film's "what if" montage, it quotes every theme, and its sections are acts. Measured once by `scripts/shows/lalaland-epilogue-onsets.py` into `scripts/shows/plans/lalaland-epilogue-onsets.json` (numpy and ffmpeg), and `check:shows` holds every strike of the show against that file:
