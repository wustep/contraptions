...
/**
 * The kit every part of Spark is built from (after Liftoff's and All at Once's).
 *
...
type Styled = { drawingContext: CanvasRenderingContext2D; _cachedFillStyle?: unknown; _cachedStrokeStyle?: unknown; _setFill?: (f: unknown) => void; _setStroke?: (s: unknown) => void; sparkHonest?: boolean }
/**
...
function honest(p: p5): void {
  ...
  if (!r || r.sparkHonest || !r._setFill || !r._setStroke) return
  r.sparkHonest = true
...
export function lay(start: { col: number; row: number; begin: number; ball: BallState }, links: Link[]): Chain {
  ...
  for (const link of links) {
    ...
    if (Math.abs(time - span) > 1e-6) console.warn(`spark: ${link.part.piece.name} lasts ${time.toFixed(4)}s of a ${span.toFixed(4)}s slot`)
    const end = laneAt(built.lane, time)
    if (Math.hypot(end.x - (built.exit[0] - 0.5), end.y - built.exit[1]) > 1e-6) console.warn(`spark: ${link.part.piece.name} ends at ${end.x.toFixed(3)},${end.y.toFixed(3)}, not at its exit`)
