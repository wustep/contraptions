...
/**
 * The kit every part of Caravan is built from (after Liftoff's and Epilogue's kits: the same contract, so a slot is
...
export interface Built<S> {
  ...
  riders?: Riders
  /**
   * The people round Andrew, wherever this part shows them: Fletcher, his father Jim, Tanner (`worlds.ts`). Each
   * span is in show seconds and may run past the part's own slot (Fletcher on his podium while the ball is in the
   * kit); at any time at most one part in the show has each of them. Positions are in the part's frame; null while
   * they are out of sight (and only when they are out of shot).
   */
  company?: Company[]
}
...
/** Who keeps Andrew company: Fletcher, his father, or Tanner. */
export type Who = 'fletcher' | 'jim' | 'tanner'
...
type Styled = { drawingContext: CanvasRenderingContext2D; _cachedFillStyle?: unknown; _cachedStrokeStyle?: unknown; _setFill?: (f: unknown) => void; _setStroke?: (s: unknown) => void; caravanHonest?: boolean }
/**
 ...
 * and paint with the gradient instead (a dark that came out beige). So the renderer Caravan draws with always sets
 * the style it is given.
 */
function honest(p: p5): void {
  const r = (p as unknown as { _renderer?: Styled })._renderer
  if (!r || r.caravanHonest || !r._setFill || !r._setStroke) return
  r.caravanHonest = true
...
export function lay(start: { col: number; row: number; begin: number; ball: BallState }, links: Link[]): Chain {
  ...
  for (const link of links) {
    ...
    if (Math.abs(time - span) > 1e-6) console.warn(`caravan: ${link.part.piece.name} lasts ${time.toFixed(4)}s of a ${span.toFixed(4)}s slot`)
    const end = laneAt(built.lane, time)
    if (Math.hypot(end.x - (built.exit[0] - 0.5), end.y - built.exit[1]) > 1e-6) console.warn(`caravan: ${link.part.piece.name} ends at ${end.x.toFixed(3)},${end.y.toFixed(3)}, not at its exit`)
