...
/**
 * The kit every part of Merry-Go-Round is built from (after Liftoff's, and All at Once's).
 *
...
export interface Built<S> {
  ...
  riders?: Riders
  /**
   * Howl (`worlds.ts`, HOWL) and Markl (MARKL), wherever this part shows them.
   * Each span is in show seconds and may run past the part's own slot; at any
   * time at most one part in the show has each of them. Positions are in the
   * part's frame; null while they are out of sight (and only when they are out
   * of shot, or at a cut, when the whole place changes). A span may set `color`
   * (Howl as the bird goes to HOWL_BIRD) and `scale` (Markl is MARKL_SCALE).
   */
  company?: Company[]
}
...
type Styled = { drawingContext: CanvasRenderingContext2D; _cachedFillStyle?: unknown; _cachedStrokeStyle?: unknown; _setFill?: (f: unknown) => void; _setStroke?: (s: unknown) => void; merryGoRoundHonest?: boolean }
/**
...
function honest(p: p5): void {
  ...
  if (!r || r.merryGoRoundHonest || !r._setFill || !r._setStroke) return
  r.merryGoRoundHonest = true
...
export function lay(start: { col: number; row: number; begin: number; ball: BallState }, links: Link[]): Chain {
  ...
  for (const link of links) {
    ...
    if (Math.abs(time - span) > 1e-6) console.warn(`merry-go-round: ${link.part.piece.name} lasts ${time.toFixed(4)}s of a ${span.toFixed(4)}s slot`)
    const end = laneAt(built.lane, time)
    if (Math.hypot(end.x - (built.exit[0] - 0.5), end.y - built.exit[1]) > 1e-6) console.warn(`merry-go-round: ${link.part.piece.name} ends at ${end.x.toFixed(3)},${end.y.toFixed(3)}, not at its exit`)
