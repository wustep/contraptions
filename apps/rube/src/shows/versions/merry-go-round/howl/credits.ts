 ...
 * comes up, holds, and goes as the next comes.
 *
 ...
 * only a soft shade under them, so they read over a bright sky.
 */
...
const script: Omit<Card, 'at'>[] = [
  ...
  },
  {
    hold: 4.0,
    role: 'After',
    names: ['Howl’s Moving Castle'],
    notes: ['a film by Hayao Miyazaki, Studio Ghibli (2004)', 'from the novel by Diana Wynne Jones'],
  },
...
/** When the last card has gone: after it the castle flies on to the end. */
...
/** The canvas's half: a soft shade where the words come, over the sky. */
...
export const CREDITS_OK = CREDITS_AT >= CHORD[2] + 1.5 && LAST_GONE <= DURATION - 2

/** Sophie's swatch at the end, for the director's finale (her colour when the credits roll). */
export const SOPHIE_END = SOPHIE_SILVER
