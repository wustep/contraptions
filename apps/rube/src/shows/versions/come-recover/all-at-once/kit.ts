...
/**
 * The kit every part of All at Once is built from (after Liftoff's, in Cornfield Chase).
 *
...
export interface Built<S> {
  ...
  riders?: Riders
  /**
   * The hero's family: Joy (`worlds.ts`, JOY) and Waymond (WAYMOND), wherever
   * this part shows them. Each span is in show seconds and may run past the
   * part's own slot (Waymond still waving by the door after Evelyn has gone on);
   * at any time at most one part in the show has each of them. Positions are in
   * the part's frame; null while they are out of sight (and only when they are
   * out of shot, or at a jump, when the whole world changes).
   */
  company?: Company[]
}
...
type Styled = { drawingContext: CanvasRenderingContext2D; _cachedFillStyle?: unknown; _cachedStrokeStyle?: unknown; _setFill?: (f: unknown) => void; _setStroke?: (s: unknown) => void; allAtOnceHonest?: boolean }
/**
...
function honest(p: p5): void {
  ...
  if (!r || r.allAtOnceHonest || !r._setFill || !r._setStroke) return
  r.allAtOnceHonest = true
...
export interface Chain {
  ...
  /** The parts' spans of Joy and Waymond, in world cells. */
  company: Company[]
...
export function lay(start: { col: number; row: number; begin: number; ball: BallState }, links: Link[]): Chain {
  ...
  for (const link of links) {
    ...
    if (Math.abs(time - span) > 1e-6) console.warn(`all at once: ${link.part.piece.name} lasts ${time.toFixed(4)}s of a ${span.toFixed(4)}s slot`)
    const end = laneAt(built.lane, time)
    if (Math.hypot(end.x - (built.exit[0] - 0.5), end.y - built.exit[1]) > 1e-6) console.warn(`all at once: ${link.part.piece.name} ends at ${end.x.toFixed(3)},${end.y.toFixed(3)}, not at its exit`)
