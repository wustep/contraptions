...
import { R, ROLL, definePiece, over, post, rail, ramp, roll, trace, type Lane, type Pt } from '../../parts'
 ...
 * down the inside to the small tube, where it is a snug fit, and its way
 ...
 *
 * The ball is at the small tube's end the whole way, so where the tip is,
 * it is: its hidden lane is sampled from the same draw and the same tilt
 * the tubes are drawn with.
 */
...
const TUBE = 0.5
/** The ball rides this far behind the tip, stuck in the small tube's end. */
const SEAT = R + 0.02
...
const FIRE = T_IN + (SHUT_TIP - SEAT - IN) / ROLL
...
const ballAt = (t: number): Pt => onGlass(tipAt(t) - SEAT, t)
const HIDDEN = [
  roll(onGlass(IN, 0), ballAt(FIRE), ROLL),
  ...trace(ballAt, FIRE, T_A, 8),
  ...trace(ballAt, T_A, T_B, 14),
  ...trace(ballAt, T_B, T_OUT, 4),
].map((s) => ({ ...s, hidden: true }))
const LANE: Lane = {
  segs: [roll([-0.5, 0], onGlass(IN, 0), ROLL), ...HIDDEN, ramp(ballAt(T_OUT), [1.5, 0], V_OUT, ROLL)],
  fire: FIRE,
}
...
export const spyglass = definePiece<{ color: string }>({
  ...
  over: (p, s, { k, t, ink, bg, weight }) => {
    ...
    p.pop()
    // The eyepiece's open end.
    p.fill(ink)
    p.noStroke()
    p.ellipse(ends[2] * k, 0, 0.045 * k, (RAD[2] + 0.02) * 2 * k)
