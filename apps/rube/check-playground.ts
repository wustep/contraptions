...
async function main(): Promise<void> {
  ...
  for (const shelf of shelves) {
    ...
    if (isWorld) {
      ...
      const expectedBeats = w.name.startsWith('underhill-') ? 7 : 18
      check(`${expectedBeats} beats and a rail of its own`, beats.length === expectedBeats && w.own!.includes('rail'), `${beats.length}`)
