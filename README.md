...
| flags | across a treadle plank; the pawl under it lets a lead weight go into the water, and three signal flags run up the mast, a pennant, a square and a swallowtail, each shaking itself out as it clears the ball; the lead gathers way and the water slows it, so the hoist starts and stops without a jolt |
...
| buoychain | off the deck's end onto a squat can buoy in its trough, the first of three on one ground chain, a can, a pillar and a spar, each half a swell behind the one before; the swell carries it up a third of a floor and heels it at the crest, and it hops from flat top to flat top; the spar tips it onto the deck a floor up |
...
| windchime | bats the sail of a wind chime up ahead of it and goes under; the sail comes down on its crown and is shoved into the first of three wooden tubes hung from a bough; the clack runs down the row and rings off its end, then back up the row and rings off the other, back and forth, softer each time; out a touch slower |
...
| web | off the path's end into a spider's web strung between two saplings, the spider up on the bridge line; the web stretches deep and quivers and the spider runs to the sapling; the threads to the bridge line snap and the ball drops through to the path a floor down; the torn web hangs, and the spider lets itself down to look |
...
| popcorn | up the chute into the kettle in a popcorn cart's case, out of sight; the element lights, the kettle shakes, kernels burst faster and faster and the case fills half way; up through the heap and out of the open top in a spray of popcorn onto the shelf a floor up; later the heap is served out again; +100 |
...
| mirrors | along the lane past a hall of mirrors: three funhouse glasses behind it, and in each as the ball passes a thing that is and is not the ball, squat in the barrel, a thin candle in the hourglass, rippling in the ribbon; each frame's lamp lights while it has somebody in it, and each glass pays as the ball goes by; +10, +20, +30 |
