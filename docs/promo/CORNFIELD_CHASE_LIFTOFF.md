...
The music is Hans Zimmer's *Cornfield Chase* and then *No Time for Caution*, both from *Interstellar* (2014). Attribution is in:
...
Every change of scene is scored and continuous. No cue or strike was moved for a transition.

**Act I**

| Time | From → to | How it carries |
| --- | --- | --- |
| 0 to 84 s | room → stairs → porch → yard → channel → truck → corn → dam → combine → base → tower | One universe and one camera. Every hand-off is the ball's own path. Brand's entrance out of the bunker happens behind a wall, so she never pops. |
| 83.8 s | the countdown → ignition | The camera pushes in on the two of them in the window as the lamps go out, and ignition throws it wide. |
...
**Why No Time for Caution.** It is the docking cue, and Act II is the docking run backwards.
- Its organ pulse holds a steady 60 bpm, so every beat is a clean target.
- It has a shape a machine can play. It starts quiet, steps up in loudness again and again, peaks, and stops dead.
- That shape gave the layout: quiet station machinery while it is soft, the undock on its biggest step, touchdown on its loudest bar, and a lamp on its last hit.

**The audio.**
- The cue was fetched once with yt-dlp and ffmpeg from WaterTower Music's official upload. It is kept in `docs/promo/interstellar-no-time-for-caution-demo.mp3`, labelled demo only.
- The show plays one file, `docs/promo/interstellar-liftoff-mix-demo.mp3`, built by `scripts/liftoff-mix.sh`:
...
**Its clock.**
- `scripts/liftoff-ntfc-onsets.py` measured the mix once (numpy and ffmpeg) into `scripts/show-plans/liftoff-ntfc-onsets.json`.
- The cue's beat *k* is at 23.5177 + 0.9999·*k* s of the show (`cue(k)` in `music.ts`). Every beat and eighth sits on that comb to a few ms.
- The file also has each beat's measured strength, and where the loudness steps up, bar by bar.
...
- **Where the targets come from.**
  - Act I:
    - the piano's own notes in the rubato opening (±40 ms);
    - the organ's onsets as it gathers (±30 ms);
...
- **What `check:shows` holds it to.**
  ...
  - Brand (the blue ball):
    ...
    - she is out of shot everywhere else (ten sample times);
    - she goes grey in orbit;
    - she never jumps;
...
Beats are No Time for Caution's (`cue(k)`). The station is drawn end-on, as a ring of land 20 cells from the axis. The camera sits in the ring's own frame, so the land is still and "down" is outward. The ball goes anticlockwise round the ring: from the replica at the bottom, up the right-hand side, and across the axis.
...
7. **The cloud deck is drawn with intent** (`cloud.ts`).
   - Three layers of cumulus, one behind another, each drifting at its own pace: a far one peeking over the top, a middle one, and a near one that makes the flat base. Each layer's tops are a few big domes with smaller puffs between; the domes catch the light and the base is in shade.
   - Each layer is inked once, round its whole outline, so the puffs read as one body.
   - The hole the rocket bores leans with the rocket. Its walls are billows of cloud in their own shade, not a straight cut.
   - The collar thrown up as the rocket goes through is soft uninked vapour that swells, drifts out and thins over about 4 s. The hole closes slowly behind.
8. **The tesseract is a journey, not a landing** (`space/gargantua.ts`).
   - He no longer comes down straight onto Murph's case. The dark opens and he dives into the lattice's depth. Murph's case is small and far, hidden behind other rooms.
   - Two rooms come at him and go round him, each the same bookcase from behind at another time, another way up, with its own turned lattice. The first is on its side in moonlight, after the books have gone, and passes on beat 190 with its frame ringing gold. The second is upside down in sepia, years before, and passes on 190½.
   - Murph's case comes up out of the depth, upright and lamplit, and grows warmer as he nears it. He is behind the lander on the last hit (191).
   - The books go from 119.67 to 120.85 s, in the opening's order and Morse. The watch's three ticks, the pull-back, the bed (124.3 s) and the wake (124.5 s) are unchanged. Strikes stay on 190 and 191, so no strike moved.
9. **No watch at Murph's house on the station.** The watch and its nail are gone from the far-side room. It appears only in the bookcase scenes.
10. **The rocking chair reads as a rocking chair** (`act2/ballpark.ts`). It is drawn side on: two rockers on one arc, turned legs with a stretcher, a thick seat, an arm on a turned post with a scrolled end, and a tall raked back of spindles under a curved crest rail, with a small quilt over it. It rocks on the rockers' arc, and Murph moves with it.
11. **No question marks.** In the opening he glances at each thing as it falls instead: a small turn toward it that settles back by the next note (`earth/house.ts`).
12. **The watch strap matches the reference.** The strap is black alligator. Each band tapers from the lugs, with scale bars and a cream running stitch when it is big enough to show them, and a shade where it turns away. Standing on the shelf, the lower band runs down to the board and the upper one curls back behind with a keeper. The size, 7:37, and the Morse second hand at 45 are unchanged.
...
- **Cooper Station.** Murph's room turns out to be the museum replica of the farmhouse. The ball hit in the station's baseball game crosses the axis and breaks a window on the far side of the ring. There he finds her old, and she comes to him before she sends him on.
- **The ending.**
