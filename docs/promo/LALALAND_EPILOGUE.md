...
Open it at `/shows/?show=la-la-land&take=fable51-epilogue`. In the Shows picker it is **Epilogue**, and under it, faint, "Directed by wustep".
...
- **Her husband is the grey ball** (`#8C8F96`), in the real club at the end only.
...
| 75.72 | the swing | the freeway | *(built; see the builder's summary below)* |
| 103.84 | the swing's loudest bar | the studio | *(built; see the builder's summary below)* |
...
- The take is **Epilogue** in the picker, with a faint "Directed by wustep" byline (`ShowVersion.director`); no model or tech-demo line.
- The credits are the page's words (`Performance.titles(t)`), since a show's canvas sets no type: Directed by Stephen Wu; Machines, drawings and code, Claude Fable 5.1; With, Sebastian (the blue ball) and Mia (the yellow ball); Music, Justin Hurwitz, "Epilogue" from La La Land (2016); Drawn with p5.js. They come up high in the frame, over the piano's lid, from the second of the last chords, and are gone before the last.
