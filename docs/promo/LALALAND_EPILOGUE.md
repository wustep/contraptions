...
- The credits are the page's words (`Performance.titles(t)`), since a show's canvas sets no type: Directed by Stephen Wu; Machines, drawings and code, Claude Fable 5.1; With, Sebastian (the blue ball), Mia (the yellow ball) and David (the grey ball); Music, Justin Hurwitz, "Epilogue" from La La Land (2016); Drawn with p5.js. They come up high in the frame, over the piano's lid, from the second of the last chords, and are gone before the last.
