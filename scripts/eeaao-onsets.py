...
The show plays docs/promo/eeaao-come-recover-demo.mp3 (built by scripts/eeaao-cue.sh) from its
first sample, so show time is the recording's time. This reads it and writes
scripts/show-plans/eeaao-onsets.json, which the parts are timed to and check:shows holds them
...
CUE = os.path.join(ROOT, 'docs/promo/eeaao-come-recover-demo.mp3')
OUT = os.path.join(ROOT, 'scripts/show-plans/eeaao-onsets.json')
SR = 22050
HOP = 256
N = 2048
...
def main():
    ...
    out = {
        'source': 'docs/promo/eeaao-come-recover-demo.mp3',
