...
check('rollOptions resets the pool and the catalog', (() => {
  const o = rollOptions({ ...defaultOptions, mode: 'cascade', solo: 'hopper', tag: 'x', catalog: true })
...
// A link to a mode that has since gone still opens: on classic, with its seed.
check('a retired mode falls back to classic', parseOptions('?mode=rube&seed=s').mode === 'classic' && parseOptions('?mode=rube&seed=s').seed === 's')
