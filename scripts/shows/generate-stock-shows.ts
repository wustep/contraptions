...
const REPORTS: Record<Work, string> = { premiere: 'premiere-arabesque/TAKE_B', clair: 'clair-de-lune/TAKE_A', 'clair-b': 'clair-de-lune/TAKE_B' }

function generate(work: Work) {
  const arabesque = work === 'premiere'
  const id = IDS[work]
  const command = COMMANDS[work]
  const plan: Arrangement[] = JSON.parse(readFileSync(`scripts/shows/plans/${work}.json`, 'utf8'))
  const maps: StockMap[] = []
  let cursor = 0
  ...
  writeFileSync(`apps/rube/src/shows/versions/${REPORTS[work]}_ARRANGEMENT.md`, lines.join('\n'))
