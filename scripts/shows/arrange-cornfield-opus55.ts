/**
 * Cornfield Chase, Opus 5.5. Stock lanes only: the arrangement is
...
const ONSETS = JSON.parse(readFileSync('scripts/shows/plans/cornfield-opus55-onsets.json', 'utf8')) as Onsets
...
function main(): void {
  ...
  writeFileSync('scripts/shows/plans/cornfield-opus55.json', JSON.stringify(maps.map((m) => ({
    world: m.world, target: m.end, pieces: m.pieces.map((p) => p.spec),
  })), null, 2) + '\n')
  writeFileSync('apps/rube/src/shows/versions/cornfield-chase/OPUS55.md', report(score, list, locked, stats))
...
function report(score: StockScore, list: Target[], locked: Locked[], stats: Stats): string {
  ...
  L.push('# Cornfield Chase, Opus 5.5', '')
  L.push('Copyrighted recording. Private tech demo and eval one-shot only — do not ship this audio in a public build.')
  L.push('Hans Zimmer, *Cornfield Chase*, from *Interstellar* (2014). Attribution: `apps/rube/src/shows/versions/cornfield-chase/ATTRIBUTION.txt`.', '')
  L.push(`Generated by \`npm run generate:cornfield:opus55\` (\`scripts/shows/arrange-cornfield-opus55.ts\`). Every machine is a stock piece at its stock duration, compiled through \`stockPlacement\` like every other stock show. The arrangement is only the order, the exit variant and the rail breaths.`, '')
  L.push('## The targets are measured', '')
  L.push(`\`scripts/shows/cornfield-opus55-onsets.py\` reads the recording once and writes \`scripts/shows/plans/cornfield-opus55-onsets.json\`; the generator never touches audio.`, '')
