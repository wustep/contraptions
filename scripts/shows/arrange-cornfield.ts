...
function main(): void {
  ...
  writeFileSync('scripts/shows/plans/cornfield.json', JSON.stringify(score.maps.map((m) => ({
    world: m.world, target: m.end, pieces: m.pieces.map((p) => p.spec),
  })), null, 2) + '\n')
  writeFileSync('apps/rube/src/shows/versions/cornfield-chase/ARRANGEMENT.md', report(score, cues, mean, strongMean, grown.misses + chased.misses))
...
function report(score: StockScore, cues: { piece: string; target: number; actual: number; section: string; strong: boolean }[], mean: number, strongMean: number, misses: number): string {
  ...
  lines.push('Attribution file: `apps/rube/src/shows/versions/cornfield-chase/ATTRIBUTION.txt`.', '')
