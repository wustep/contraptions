...
const plan = JSON.parse(readFileSync(`scripts/shows/plans/${work}.json`, 'utf8')) as { world: string; target: number; pieces: unknown[] }[]
...
if (work === 'clair-b') {
  ...
  const takeAPlan = JSON.parse(readFileSync('scripts/shows/plans/clair.json', 'utf8')) as typeof plan
