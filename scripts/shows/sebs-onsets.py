...
The show plays apps/rube/src/shows/versions/la-la-land/la-la-land-sebs-mix-demo.mp3 (built by
scripts/shows/sebs-mix.sh): Justin Hurwitz's Epilogue whole from zero, then The
End from 464.0 s. This reads the mix and writes
scripts/shows/plans/sebs-onsets.json, which the parts are timed to and
check:shows holds them against. Rerun only if the mix changes:

    python3 scripts/shows/sebs-onsets.py
...
MIX = os.path.join(ROOT, 'apps/rube/src/shows/versions/la-la-land/la-la-land-sebs-mix-demo.mp3')
OUT = os.path.join(ROOT, 'scripts/shows/plans/sebs-onsets.json')
SR = 22050
HOP = 256
N = 2048
# Where The End comes in (scripts/shows/sebs-mix.sh).
END_AT = 464.0
...
def main():
    ...
    out = {
        'source': 'apps/rube/src/shows/versions/la-la-land/la-la-land-sebs-mix-demo.mp3',
