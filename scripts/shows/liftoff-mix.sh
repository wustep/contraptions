...
#
#   sh scripts/shows/liftoff-mix.sh <no-time-for-caution>
#
# <no-time-for-caution> is the cue as fetched from the official upload (WaterTower Music), which is not kept:
#
...
# (see apps/rube/src/shows/versions/*_ATTRIBUTION.txt). The mix is what the show plays; this
...
set -e
NTFC="$(cd "$(dirname "${1:?No Time for Caution, as fetched}")" && pwd)/$(basename "$1")"
cd "$(dirname "$0")/../.."
ffmpeg -loglevel error -y \
  -i apps/rube/src/shows/versions/cornfield-chase/cornfield-chase-zimmer.mp3 \
  -i "$NTFC" \
  -filter_complex "\
