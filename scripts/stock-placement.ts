 ...
 * The default payout preserves older takes. New arrangements pass their accumulated points. */
export function stockPlacement(world: World, spec: StockSpec, ball: BallState, index: number, earned = 600) {
  ...
  const placed = spec.portal ? portalPlacement(spec.portal, color) : stock.place({
    rng: makeRng('stock'), color, theme, ball, earned, taste: { weights: {} },
