...
The arrangement (`scripts/arrange-cornfield-opus55.ts`) reads the JSON this
writes and never the audio, so the build needs no Python. Rerun only if the
recording changes:

    python3 scripts/cornfield-opus55-onsets.py
...
MP3 = os.path.join(ROOT, 'docs/promo/cornfield-chase-zimmer.mp3')
OUT = os.path.join(ROOT, 'scripts/show-plans/cornfield-opus55-onsets.json')
SR = 22050
...
def main():
    ...
    out = {
        'source': 'docs/promo/cornfield-chase-zimmer.mp3',
