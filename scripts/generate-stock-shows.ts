...
function generate(work: 'premiere' | 'clair') {
  const arabesque = work === 'premiere'
  const id = arabesque ? 'premiere-arabesque/take-b' : 'clair-de-lune/take-a'
  ...
  const phrases: Phrase[] = arabesque
    ...
    : clairPhrases.map(([end, title, visible], i) => ({ begin: i ? clairPhrases[i - 1][0] : 0, end, title, visible }))
  const pieces = maps.flatMap((m) => m.pieces)
  const score: StockScore = { id, title: arabesque ? 'Première Arabesque' : 'Clair de Lune',
    performer: arabesque ? 'Patrizia Prati' : 'Laurens Goedhart', audioOffset: arabesque ? 2.38 : 2.44,
  ...
  const lines = [`# ${score.title}, ${id.split('/')[1]}`, '',
    `Generated by \`npm run generate:${work === 'premiere' ? 'premiere:b' : 'clair'}\`. Every piece runs at stock timing.`, '',
  ...
  writeFileSync(`docs/promo/${work === 'premiere' ? 'PREMIERE_TAKE_B' : 'CLAIR_TAKE_A'}_ARRANGEMENT.md`, lines.join('\n'))
